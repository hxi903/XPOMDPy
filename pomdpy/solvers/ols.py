...
# credits: Lucas Allegre. source: https://github.com/LucasAlegre/sfols/blob/main/rl/successor_features/ols.py
...
class OLS:
    ...
    def add_solution(self, value, w, gpi_agent=None, env=None) -> int:
        ...
        self.iteration += 1
        self.W.append(w)
        # checking if new policy (value) is showing improvement of current CSS
        ...
        # do these lines below for new improving policy (value)
    ...
    def remove_obsolete_values(self, value: np.ndarray) -> List[int]:
        ...
        for i in reversed(range(len(self.ccs))):
            ...
            if best_in_all:
                ...
                self.ccs.pop(i)
                print(f"removing {self.ccs_weights[i]}")
                self.WA.pop(str(self.ccs_weights.pop(i)))
    ...
    def max_value_lp(self, w_new: np.ndarray) -> float:
        ...
        return prob.solve(solver="SCS", verbose=False)
    ...
    def corner_weight(self, v_new: np.ndarray, v_set: List[np.ndarray]) -> np.ndarray:
        ...
        prob = cp.Problem(objective, constraints)
        prob.solve(solver='SCS', verbose=False)  # (solver='SCS', verbose=False, eps=1e-5)
        if prob.status == cp.OPTIMAL:
    ...
    def plot_ccs_xy(self, ccs, ccs_weights, gpi_agent=None, eval_env=None):
    ...
    def plot_ccs_xyz(self, ccs, ccs_weights, gpi_agent=None, eval_env=None):
        import seaborn as sns
        import matplotlib.pyplot as plt
        ...
        for i in range(len(x)) :  # plot each point + it's index as text above
            ...
            #ax.text(x[i], y[i], z[i], '%.2f,%.2f,%.2f' % (x[i],y[i],z[i]), size=10, zorder=1,
                    #color='k')
            ax.text(x[i], y[i], z[i], '%s' % (str(ccs_weights[i])), size=10, zorder=1,
                    color = 'k')
            #plt.savefig(f"figs/ccs_dst{self.iteration}.pdf", format="pdf", bbox_inches="tight")
        plt.show()

    def plot_ccs_basic_xy(self, ccs, ccs_weights, gpi_agent=None, eval_env=None):
        import seaborn as sns
        import matplotlib.pyplot as plt
        from mpl_toolkits.mplot3d import Axes3D
        ...
        for i in range(len(x)) :  # plot each point + it's index as text above
            ...
            ax.text(x[i], y[i], '%s' % (str(ccs_weights[i])), size=10, zorder=1,
                    color='k')
            #plt.savefig(f"figs/ccs_dst{self.iteration}.pdf", format="pdf", bbox_inches="tight")
        plt.show()
