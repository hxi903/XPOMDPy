...
class ValueIteration(Solver):
    ...
    def value_iteration(self, t, o, r, horizon):
        ...
        for k in range(horizon):
            ...
            Q_temp = self.gamma.copy()
            for u in range(actions):
                Q_u = set()
                for av in Q_temp:
                    if av.action == u:
                        Q_u.add(av)
                prunedQ_u = self.prune_av(Q_u, states)
                LastQ.update(prunedQ_u)
            #self.prune(states)
            self.gamma = LastQ
    ...
    def prune_av(self, vector_set, n_states):
        """
        Remove dominated alpha-vectors using Lark's filtering algorithm
        :param n_states
        :return Q containing pruned alpha-vectors:
        """
        # parameters for linear program
        delta = 0.0000000001
        # equality constraints on the belief states
