...
def getVectorValue(belief: list , V: list[AlphaMatrix] , w: np.ndarray ):
    print(f"calculating vector value")
    max = float('-inf')
    max_vms = []
...
def getBestAlphaMatrices(A_all: list[AlphaMatrix], B: list[BeliefPoint], w: np.ndarray):
    Ar = []
    seen = []
    for b in B:
      m_idx=getBestMatrixIndex(b.getBelief(), A_all, w)
      # avoid duplicate AlphaMatrices in Ar
      if A_all[m_idx].__hash__() not in seen:
          Ar.append(A_all[m_idx])
          seen.append(A_all[m_idx].__hash__())
    return Ar