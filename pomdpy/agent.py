...
class Agent:
    ...
    def discounted_return(self):
        ...
        elif self.model.solver == 'PerseusPBVI':
            solver = self.solver_factory(self)

            self.run_perseus_pbvi(solver,1)
        elif self.model.solver == 'PerseusOLS':
            solver = self.solver_factory(self)
    ...
    def prepareBelief(self, b: BeliefPoint):
        assert b != None
        ...
        aoProbs = np.zeros((nActions, nObservations))

        for a in range(nActions):
            for o in range(nObservations):
                prob=0
                for sNext in range(nStates):
                    p=0
                    for s in range(nStates):
                        p += T[a][s][sNext] * b.getBeliefFromState(s)
                    prob += O[a][sNext][o] * p
                aoProbs[a][o] = prob
        b.setActionObservationProbabilities(aoProbs)
    def random_explore(self, n: int, b0):
        B = []
        Bset = set()
        b = BeliefPoint(b0)
        B.append(b)
        ...
        while len(B) < (n - self.model.num_states):
            ...
            for o in range(self.model.num_observations):
                prob = b.getActionObservationProbability(action, o)
            ...
            bao = self.updateBelief(b, action, observation)
        ...
        for s in range(self.model.num_states):
            beliefEntries= np.zeros(self.model.num_states)
            beliefEntries[s]=1.0
            B.append(BeliefPoint(beliefEntries))

        return B

    def updateBelief(self, b: BeliefPoint, a, o):
        assert a < self.model.num_actions and o < self.model.num_observations
        T = self.model.get_transition_matrix()
        O = self.model.get_observation_matrix()
        newBelief = np.empty((self.model.num_states))

        # check if belief point has been prepared
        if not b.hasActionObservationProbabilities():
            self.prepareBelief(b)

        # compute normalising constant
        nc = b.getActionObservationProbability(a, o)
        assert nc > 0.0, "o can not be observed when executing a in belief b"

        # compute the new belief vector
        for sNext in range(self.model.num_states):
            beliefEntry = 0.0

            for s in range(self.model.num_states):
                beliefEntry += T[a][s][sNext] * b.getBeliefFromState(s)

            newBelief[sNext] = beliefEntry * O[a][sNext][o] / nc

        return BeliefPoint(newBelief)
    ...
    def run_perseus_ols(self, solver, epoch):
        ...
        print("=== RUN Perseus OLS MR-POMDP/MO-POMDP SOLVER ===")
        print("Belief sampling started ...")
        b0 = self.model.get_initial_belief_state()
        print(f"initial belief = {b0}")

        # RandomExplore(n) (see Alg.4)
        assert self.model.sampling_belief_size >= self.model.num_states, "error: sampling belief size at least equal to the number of state"
        ...
        solver.perseus_ols(self.model.get_transition_matrix(),
                            self.model.get_observation_matrix(),
    ...
    def run_perseus_pbvi(self, solver, epoch) :
        ...
        print("=== RUN Perseus PBVI POMDP SOLVER ===")
        print("Belief sampling started ...")
        ...
        solver.perseus_pbvi(self.model.get_transition_matrix(),
                               self.model.get_observation_matrix(),
                               self.model.get_reward_matrix(), B)
        ...
        for i in range(self.model.max_steps) :
            ...
            # TODO: record average V(b) per epoch
            action, v_b = solver.select_action(b0, solver.gamma)
            step_result = self.model.generate_step(action)

            if not step_result.is_terminal :
                b0 = self.model.belief_update(b0, action, step_result.observation)
                print(f"updated belief = {b0}")
