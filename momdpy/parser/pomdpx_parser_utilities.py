...
Source: https://github.com/iciac/POMDP/blob/master/src/parser_main.py
...
def iterable(obj) :
    '''
    helper function for get_reward_function to remove empty list
    '''
    try :
        iter(obj)
    except Exception :
...
def remove_empty_list(array) :
    ...
    if scalar :
        ...
        if len(array) == 0 :
            ...
            y = [remove_empty_list(i) for i in array]
    return y
...
def get_reward_function(root): 
    ...
    for child in root.findall('RewardFunction'):
        for func in child.findall('Func'): 
            ...
            for parent in func.findall('Parent'): 
                ...
                reward_table = np.frompyfunc(list, 0, 1)(np.zeros((dims), dtype=object)) # making multi-dimensional empty lists
    ...
    return remove_empty_list(reward_table)
...
def get_numpy_reward(root,parent,entry,instance, numpy_array):
    ...
    if len(value_split) > 1:
        numpy_array[index_list] = [float(idx) for idx in value_split]
    else:
        numpy_array[index_list] = float(value)
    return numpy_array
...
def import_policy_vector(root):
    ...
    for i in root.findall('AlphaVector'):
        ...
        for m in i.findall('Vector'):
            ...
            obsValue_list.append(int(m.attrib['obsValue']))
    ...
    return policy_vectors_field, best_action_list, obsValue_list

def import_policy_matrix(root):
    '''
    This function uses the root of a parsed XML policy file, parses it and returns two lists.
    First list contains policy vectors.
    Second list contains the best actions for each policy.
    ...
    for i in root.findall('AlphaMatrix'):
        policy_vectors_field = []
        best_action_list = []
        obsValue_list = []
        for m in i.findall('Matrix'):
            best_action_list.append(int(m.attrib['action']))
            obsValue_list.append(int(m.attrib['obsValue']))
            pom = m.text.rstrip(' ').split(';')
            policy_list = []
