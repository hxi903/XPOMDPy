...
TODO:
-alphamatrices to policy for MR-MOMDP
"""
...
def alphavectors_to_policy(AlphaVectorSet: list, no_entries: int, filename: str):
    ...
    element1.set('numObsValue', str(len(AlphaVectorSet)))
    numVector = reduce(lambda count, l : count + len(l), AlphaVectorSet, 0)
    element1.set('numVectors', str(numVector))

    i=0
    for x in AlphaVectorSet:
        for av in x:
            ...
            s_elem1.set('obsValue', str(i))  # observable state
...
def alphamatrices_to_scalarized_policy(AlphaMatrixSet: list, no_entries: int, weights: list, filename: str):
    ...
    element1 = ET.SubElement(data, 'AlphaVector')
    element1.set('vectorLength', str(no_entries))
    element1.set('numObsValue', str(len(AlphaMatrixSet)))
    numVector = reduce(lambda count, l : count + len(l), AlphaMatrixSet, 0)
    element1.set('numVectors', str(numVector))

    i=0
    for x in AlphaMatrixSet:
        for av in x:
            # Adding subtags under the `AlphaVector`
            # subtag
            s_elem1 = ET.SubElement(element1, 'Vector')
...
if __name__ == "__main__":
    ...
    a3 = AlphaVector(2, np.array([0.675, 0.754]))
    a4 = AlphaVector(1, np.array([0.125, 0.977]))
    a5 = AlphaVector(2, np.array([0.1235, 0.1547]))

    A=[[],[],[]]
    A[0]=[a1, a2]
    A[1]=[a3, a4]
    A[2]=[a5]
    alphavectors_to_policy(A, 2, "test_momdp.xml")
    ...
    A[1]=[a3, a4]
    A[2]=[a5]
    alphamatrices_to_policy(A, 3, "test_mrmomdp.xml")
    alphamatrices_to_scalarized_policy(A,3,weights,'test_scalarised_mrmomdp.xml')
